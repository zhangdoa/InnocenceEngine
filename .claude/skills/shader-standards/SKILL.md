...
description: Use when writing engine HLSL. Row-major matrices, identity-fallback transforms, cross-queue UAV DeviceMemoryBarrier, *WithGroupSync uniformity (no early return).
---

# Skill: shader-standards
...
- `safety-observability` — *no magic numbers* and *no copy-paste* apply to HLSL.
- `fundamentals` — *fail loudly* and *explicit contracts* are the underlying quality bar.
