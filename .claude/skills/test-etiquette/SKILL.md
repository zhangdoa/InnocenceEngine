---
name: test-etiquette
description: Use before launching the engine, editor, Playwright, or any long-running script. Launch budget in plan, read-first-run-last, serialize Playwright via npm test, sweep orphans between iterations.
---

# Skill: test-etiquette
...
- `agent-dispatch` (user level) — protects user attention via dispatcher-surface occupancy; this one via launch budgets.
- `perf-frame-budget` — paired for engine launches; pick N from the perf bucket.
- `regression-build-chain` — bisect steps require user-verified launches.
