---
name: paper-port
description: Use when implementing an algorithm from a published source. Reference implementation is the source of truth; verify paper preconditions match engine state before deploying.
---
...
- `tech-choice` (user level) — runs first when the choice is *which technique*.
- `fundamentals` — paper-port is the cite-before-invent special case for published papers.
