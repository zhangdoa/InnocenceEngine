...
When the user reports a regression — *"X used to work, now it's broken"* — the first action is to identify the breaking commit. Not to dispatch a fix. A fix dispatched on top of an unreproduced regression piles new variables onto an already-uncertain baseline, contaminates the bisect surface, and makes the actual root cause harder to find.
...
### clangd index contamination — C++ analog
...
- `tech-choice-vs-default.md` — the *understand why* step (#5 above) often surfaces a wrong-tech-pick that should have been caught before ship; that discipline is the design-time form.
