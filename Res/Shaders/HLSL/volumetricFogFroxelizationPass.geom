...
float2 getHalfPixelSize(int axis)
{
	if (axis == 0)
	{
		return float2(1.0 / 64.0, 1.0 / 90.0);
	}
	else if (axis == 1)
	{
		return float2(1.0 / 160.0, 1.0 / 64.0);
	}
	else
	{
		return float2(1.0 / 160.0, 1.0 / 90.0);
	}
}
...
void main(triangle GeometryInputType input[3], inout TriangleStream<PixelInputType> outStream)
{
	...
	matrix VP = volumetricPassCBuffer.VP[selectedIndex];
	matrix VP_inv = volumetricPassCBuffer.VP_inv[selectedIndex];
	...
	float2 halfPixel = getHalfPixelSize(selectedIndex);
	...
	for (int j = 0; j < 3; ++j)
	{
		output.posCS = mul(VP_inv, pos[j]);
		output.posCS /= output.posCS.w;
		// expanded aabb for triangle
		// output.AABB = AABB(pos[j], halfPixel);
