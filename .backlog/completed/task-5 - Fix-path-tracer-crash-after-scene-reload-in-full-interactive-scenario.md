...
Bug no longer reproduces. The scene unloading callback already calls WaitForGPUIdle before deleting geometry buffers, and the ObjectStatus::Suspended guard prevents command list recording when buffers are null. Full interactive test (camera + path tracer toggle on/off/on/off + scene reload + path tracer toggle) passes consistently.
