...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Decision documented (design note or CLAUDE.md update) with rationale
- [ ] #2 If Option A: import pipeline copies textures to Data/ at import time
- [x] #3 If Option B: document that OriginalAssets/ is a runtime dependency and update onboarding docs
...
Decision: Option B (reference OriginalAssets at import time; runtime uses .innobin). Rationale: texture binary files (.innobin) are already generated in Data/Generated/Components/ at import time and contain raw pixel data. At runtime, STBWrapper::Load reads these .innobin files — the original PNGs in OriginalAssets/ are an IMPORT-ONLY dependency. This is effectively already implemented. OriginalAssets/ is documented as required only for re-import operations. When TASK-10 (BC compression) is implemented, the .innobin files will be replaced by .dds files, also stored in Data/Generated/Components/.
