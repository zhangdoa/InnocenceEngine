...
EXTENSION (surfaced 2026-05-02 during TASK-77.1 secondary-bounce-write peer review by `graphics-api-expert`; same task-status carry-forward — Done): the GISponza nondeterminism is broader than hash variance from late-binding TLAS rebuilds. The **GISponza startup camera position itself shifts across binary instantiations** — same command, same binary, the auto-test driver picks visibly different camera viewpoints at frame 30 across runs. This is GISponza-only; GITestBox and UnitTest cameras are deterministic across runs. Implication: even a "clean" run-to-run visual A/B against a GISponza baseline can fail purely because the camera is somewhere else — geometric viewpoint variance, not just shading variance. The next PT A/B owner targeting GISponza must either pin the camera explicitly (e.g. `-camera_orbit` with fixed seed-equivalent params) or fall back to UnitTest/GITestBox for hash-stable A/B. Originating peer review: TASK-77.1 secondary-bounce-write (rendering-researcher CL on `ecs-overhaul`, parallel to this commit).

EXTENSION (surfaced 2026-05-02 during TASK-77.1 mip-aware Site-3 read peer review by `graphics-api-expert`; same task-status carry-forward — Done): the cross-binary nondeterminism documented above (TLAS-race PNG-hash variance, then GISponza camera-position drift) **generalises to all three test scenes**, not just GISponza. Cross-CL toggle=0 captures (this CL's toggle=0 vs prior CL `d1f8feb5`'s toggle=0) differ by **MAE 37 across UnitTest, GITestBox, AND GISponza**. Within-binary determinism is preserved (repeat captures of the same binary at toggle=0 differ by MAE 0.003 — effectively bit-identical modulo the existing TLAS race). Implication: a "fresh-rebuild visual A/B against a baseline binary" is structurally unreliable for ANY of the three test scenes. Within-binary toggle A/B (toggle=0 vs toggle=1 in the same binary) remains usable for per-CL discrimination and is the recommended gate shape until the underlying cross-binary state-divergence is rooted out. Originating peer review: TASK-77.1 mip-aware Site-3 read (rendering-researcher CL on `ecs-overhaul`, parallel to this commit).
<!-- SECTION:NOTES:END -->
