...
- **Shader cache hygiene**: TASK-146 is in flight. Every iteration must `rm -rf Bin/Shaders/DXIL/ Bin/RelWithDebInfo/Shaders/DXIL/` then rerun `Build/HLSL2DXIL_NoPause.ps1` then `cmake --build`. See `.claude/disciplines/regression-fix-flow.md` § "Build-cache contamination". When TASK-146 lands, this becomes unnecessary.
