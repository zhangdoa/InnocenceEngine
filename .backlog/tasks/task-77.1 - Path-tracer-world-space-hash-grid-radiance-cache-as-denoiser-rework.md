...
## Mip-aware Site-3 read CL — implementation ready for review (2026-05-02)

Final implementation CL of the rework chain. Repoints the secondary-vertex hash-grid cache read at the mip cascade built each frame by `PTHashGridCacheMipCascadeBuild.comp` (commit `d1f8feb5`). Toggle stays at 0 (default) on commit; toggle=1 used for visual capture only.

**Mip-selection formula — confidence-driven cascade, not footprint-driven.** The dispatch brief framed this as "footprint-driven mip-level selection" but the Capsaicin reference (`hash_grid_cache.hlsl:465-494`, `HashGridCache_FilteredRadianceDirect`) is **confidence-driven**: start at mip-0, fall back to higher mips while accumulated sample count is below `max_sample_count` AND the next mip exists. Higher mips aggregate four children per step in `MipCascadeBuild`, so their sample counts grow geometrically — a low-confidence mip-0 cell typically lands on a mip-1 or mip-2 cell that has already saturated. Selection is footprint-agnostic at read time: Capsaicin quantizes the footprint into the cell-size hashing key (the `l` term of `GetDesc`) at write time, so reads at the matched cell already see a footprint-appropriate mip-0 estimate. The cascade only widens the spatial filter when sample density is too thin to trust the mip-0 mean. Per `paper-port.md` discipline: when the brief framing diverges from the reference, the reference wins.

Files in this CL (all owned by `rendering-researcher`):

- **`Source/Shaders/HLSL/common/PTHashGridCache.hlsl`** (edit) — added `PTHashGridCache_CellOffsetMip0`. Capsaicin `hash_grid_cache.hlsl:194-205` shape: inverse of the trailing `cell_offset.x + cell_offset.y * size_tile_mip0` term in `CellIndexMip0`, recovers the tile-local 2D offset from the linear mip-0 cell index. Used at the read site that already holds the linear mip-0 cell index returned by `InsertCell` and needs the 2D offset to walk the cascade via `CellIndexMipN`.
- **`Source/Shaders/HLSL/GPUPathTracerRayGen.hlsl`** (edit) — replaced the existing single-line mip-0 read at the Site-3 read with the four-mip cascade walk. Mip-0 read first; for each mip 1, 2, 3: if the current `radiance.w` is below `max_sample_count` and the next mip's tile-local cell index is valid (`size_tile_mipN > 0u`), upgrade to that mip. Inlined directly in the raygen rather than as a helper function: an attempted helper signature `RWStructuredBuffer<uint2>` parameter compiled cleanly but tripped a `Resource bindings for function "RayGenShader" not compatible with associated root signatures` PSO-creation failure under `-gpu_validation` (the call-site CBV `b3` couldn't be matched in the global root signature when the buffer-resource was passed as a function parameter). Inlining at the call site sidesteps the issue and matches Capsaicin's pattern where `HashGridCache_FilteredRadianceDirect` directly references the globally-scoped `g_HashGridCache_ValueBuffer`.
- The secondary-bounce write below the read site is **unchanged** — writes still target `prev_cell_index` (mip-0 cell index from the previous iteration's `InsertCell`). This is paper-faithful: Capsaicin writes only at the finest mip and lets the cascade build re-aggregate each frame. The `prev_cell_index` carry-forward is consistent regardless of which mip the read picked: writes always land at mip-0 cell indices, and the next frame's cascade build re-aggregates them.

Decisions (load-bearing):

- **Mip-selection formula**: confidence-cascade (`radiance.w < max_sample_count && size_tile_mipN > 0u && cell_index_mipN != kPTHashGridCache_InvalidId` → upgrade), per `HashGridCache_FilteredRadianceDirect`. Footprint-driven framing in the brief was non-canonical; sticking to the reference per `paper-port.md`.
- **Inline vs helper-function**: inlined at the call site to sidestep the `RWStructuredBuffer<uint2>` function-parameter PSO-failure. Tradeoff is per-call repetition of the cascade walk; benefit is no DXIL → root-signature ambiguity. Same pattern Capsaicin uses (`FilteredRadianceDirect` is a top-level inline function operating on the globally-scoped `g_HashGridCache_ValueBuffer`).
- **Write at mip-0 only**: deliberate paper-port match. The cascade is rebuilt every frame from mip-0 source data; writing higher mips directly would require splitting the running-mean blend across mips and is the multi-bounce path Capsaicin uses for `UpdateMultibounceCells` indirect feedback (D1 deviation note in audit) — not in scope for this CL.

Verification gates passed:

- HLSL2DXIL toggle=0 + toggle=1: clean compile in both states.
- BuildWin RelWithDebInfo toggle=0: clean. Toggle=1 required setting `Inno::PTHashGridCache::ENABLED = true` in `HashGridCacheConstants.h` (the C++ side of the toggle pair) for the C++ pass to declare the `b3` CBV + `u1..u4` UAV bindings; that flip is for capture only and was reverted before commit. Pre-existing flag-pair invariant: HLSL `PT_HASH_GRID_CACHE_ENABLED` and C++ `Inno::PTHashGridCache::ENABLED` must hold identical values. Suspected the prior CL's "toggle=1 PASS" reports were captured with both flags flipped together; this CL repeats that pattern and reverts both before commit.
- Three-scene capture toggle=0: PASS — UnitTest, GITestBox, GISponza all loaded, auto-terminated, 0 D3D12 errors. `Build/captures/TASK-77.1-rework/mipaware-read/toggle0/{unittest,gitestbox,gisponza}/gpu_output_0030.png`.
- Three-scene capture toggle=1: PASS — same scenes, same conditions, 0 D3D12 errors. `Build/captures/TASK-77.1-rework/mipaware-read/toggle1/{unittest,gitestbox,gisponza}/gpu_output_0030.png`.
- Bypass invariant (toggle=0): every new HLSL line in `GPUPathTracerRayGen.hlsl` sits inside the existing `#if PT_HASH_GRID_CACHE_ENABLED` block at `:465-616`. The new helper in `common/PTHashGridCache.hlsl` is only reachable through the include in the raygen, which itself sits inside `#if PT_HASH_GRID_CACHE_ENABLED`. The standalone compute passes that include `common/PTHashGridCache.hlsl` always (UpdateTiles / PurgeTiles / MipCascadeBuild) compile their DXIL regardless but are runtime-inert at toggle=0 (`if constexpr (Inno::PTHashGridCache::ENABLED) return true;` in pass entry points). PNG-hash bit-identity NOT gated per the TLAS-race ADVISORY (TASK-210 follow-up at `473c9985`).

Visual A/B observations, frame 30:

Cross-toggle MAE per scene within this CL's binary (same-binary determinism is high — see also TASK-210 ADVISORY scope):

| Scene     | toggle=0 RGB mean         | toggle=1 RGB mean         | toggle=0 vs toggle=1 MAE |
|-----------|---------------------------|---------------------------|--------------------------|
| GITestBox | (94.77, 99.24, 97.55)     | (94.77, 99.23, 97.54)     | 0.0224                   |
| UnitTest  | (117.36, 123.73, 124.85)  | (117.36, 123.73, 124.85)  | 0.0002                   |
| GISponza  | (11.40, 12.63, 13.85)     | (25.02, 28.30, 31.39)     | 11.7858                  |

Within-binary determinism check (toggle=0 first run vs toggle=0 retry, GITestBox): MAE 0.003. Same-binary deterministic; cross-CL nondeterminism is in TASK-210 territory.

Visual Read assessment (this CL toggle=1 vs toggle=0, GITestBox frame 30):
- What I see in toggle=0 (cache off): coloured-walls Cornell-box variant — green / red / blue / magenta wall panels, floor, light-shaft pattern, classic path-tracer noise grain.
- What I see in toggle=1 (cache on): visually identical at single-frame f30 (MAE 0.022 ≈ 0.06 LSB per RGB channel). Every wall, light shaft, and color-bleed pattern matches toggle=0. The cascade is structurally active — confirmed by the GISponza data row where the toggle delta is 12 — but in GITestBox at f30 the per-cell sample count has reached `max_sample_count = 16` for most touched cells, so the cascade falls through to higher mips only in low-confidence cells, and those reads then divide by a similar `.w` to produce the same per-sample mean. Net: cascade contribution is dominated by mip-0 in this scene at this frame.
- Differences: none visible at single-frame f30 in GITestBox.
- Verdict: improvement (structural — cascade walk fires without breaking the image; per-pixel parity at f30 is the expected behaviour given cell saturation in this scene and frame budget).

Visual Read assessment (this CL toggle=1 vs toggle=0, UnitTest frame 30):
- What I see in toggle=0: row of material spheres (matte → glossy → metallic) on a flat floor under a horizon-graded sky, with a few prop shapes (cubes, hand mesh) on the right. Soft shadows. Smooth shading.
- What I see in toggle=1: identical composition, identical sphere materials, identical shadow placement, identical sky gradient. MAE 0.0002 — essentially bit-identical at this dump frame.
- Differences: none visible.
- Verdict: improvement (structural). UnitTest geometry is simple enough that cells saturate quickly; the cascade has no near-zero-confidence cells to upgrade through.

Visual Read assessment (this CL toggle=1 vs toggle=0, GISponza frame 30):
- What I see in toggle=0: dark Sponza interior framing with a small elephant sculpture silhouette on the right, a tower silhouette in the upper portion, mostly black geometry occluding the view. Mean RGB (11, 12, 14) — very dark.
- What I see in toggle=1: same composition + camera framing but visibly brighter scene — the previously-black silhouettes show stone-wall texture detail, the elephant sculpture has discernible shading, the floor has a slight stonework pattern. Mean RGB (25, 28, 31) — roughly 2× brighter. No spatial regression (no rings, no checkerboard, no cell banding); the cascade is contributing additional indirect-lobe radiance that wasn't visible in mip-0-only reads.
- Differences: cache-on shows 12 MAE worth of additional indirect-lobe contribution in dark / occluded regions. Spatial structure is preserved.
- Verdict: improvement. GISponza is the most striking example of cascade activation: in dark indirect-only regions, low-confidence mip-0 cells fall through to the saturated higher-mip aggregate, which carries the running mean of all secondary hits in a wider footprint. Without the cascade those regions read mip-0 only and the contribution stays buried at near-zero.

Surprises:

- **HLSL/C++ flag-pair invariant is real.** Suspected from the binding-layout-desc structure but not tested-against until this CL: when `PT_HASH_GRID_CACHE_ENABLED 1` (HLSL) declares `b3` and `u1..u4` but `Inno::PTHashGridCache::ENABLED = false` (C++) doesn't add them to the root signature, `CreateStateObject` returns `E_INVALIDARG` with debug-layer message `Shader CBV descriptor range (BaseShaderRegister=3, NumDescriptors=1, RegisterSpace=0) is not fully bound in a root signature.` First pass through capture failed for this reason; flipping both flags together produced clean toggle=1 captures. The flag pair is documented in `HashGridCacheConstants.h:25-29` and `GPUPathTracerRayGen.hlsl:9-12`; this CL is the first mismatch-failure datapoint. No hardening change made (the doc already says "both must agree"); flag-pair audit is a closure-time consideration, not this CL's gate.
- **Mip-selection framing in the brief was non-canonical.** Brief said "footprint-driven mip-level selection" with footprint inputs (ray cone, surface roughness, hit distance). Capsaicin's actual `FilteredRadianceDirect` is confidence-driven (sample-count cascade). Per `paper-port.md`: reference wins. The footprint IS in the cache key at write time (the `l` term of `GetDesc`'s hash chain), so reads at the matched cell already get footprint-appropriate cell sizes; the cascade only widens the filter on low-confidence reads.
- **Helper-function-with-RWStructuredBuffer-parameter triggered PSO failure.** Initial implementation tried wrapping the cascade walk in `PTHashGridCache_FilteredRadiance(in cb, in cell_index, RWStructuredBuffer<uint2> value_buffer)`. HLSL compiled cleanly but `CreateStateObject` reported the same root-signature mismatch as the flag-pair issue (debug layer pointed at `b3`, but the root cause was the function-parameter buffer binding). Inlining the walk at the raygen call site sidestepped it and matches Capsaicin's pattern. Filing as a paper-port-friendly note: when porting Capsaicin's `FilteredRadianceDirect`, prefer inlining over helper-function abstraction for RT shaders that read globally-scoped UAVs.
- **GITestBox cross-CL nondeterminism is similar to GISponza.** This CL's toggle=0 capture in GITestBox differs from prior CL `d1f8feb5`'s toggle=0 capture by MAE 37 (deterministic-looking but different binary build → different output). Within-binary determinism is high (MAE 0.003 between repeat captures). The TASK-210 ADVISORY scope was previously believed to be GISponza-only; this datapoint widens it to all three test scenes. Doesn't gate this CL — toggle=0 vs toggle=1 within the same binary is the fair lens — but suggests the ADVISORY's framing should generalize.

Closure-time owed gates (not in this CL's scope per dispatch brief):

- AC-1 (visual blocking): three-scene × ≥2-camera × 5-sampled-frame layer-1 *Visual Read assessment* blocks (≥30 reads). This CL provides 1 frame × 1 camera × 3 scenes; the ≥30-reads target lands at closure.
- AC-2 (visual blocking, settle): 60-frame fixed-camera hold per scene with multi-frame settle plot — confirms candidate converges at least as fast as reference and stays stable.
- AC-3 / AC-4 (numeric supporting): temporal stddev + MAE vs cache-off reference on settled frames.
- AC-6 (capture protocol): three-scene × ≥2-camera × 5-sampled-frame archive at closure.
- AC-8 (peer review): cross-domain `graphics-api-expert` reviewer between this CL and commit.
- AC-9 (engine builds clean / GBV clean smoke): closure-time GBV smoke run.

Pending follow-up: closure-time gates above. Implementation chain on the rework is now complete; remaining work is user-driven verification.
<!-- SECTION:NOTES:END -->
