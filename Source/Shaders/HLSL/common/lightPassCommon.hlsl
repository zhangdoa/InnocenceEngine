...
//
// TASK-205: after the GBuffer-mode trim, raw-channel writers (sun shadow,
// tile heatmap) no longer sit at the high end of the enum — they interleave
// with the lit-composite modifiers (DirectOnly = 1, IndirectOnly = 2,
// SunShadowVisibility = 3, TileLightCountHeatmap = 4). The former `>=`
// sentinel pattern stops working; use explicit set membership instead.
bool DebugViewModeReplacesRT0(uint in_DebugViewMode)
{
	return in_DebugViewMode == DEBUG_VIEW_SUN_SHADOW_VISIBILITY
		|| in_DebugViewMode == DEBUG_VIEW_TILE_LIGHT_COUNT_HEATMAP;
}
