// shadertype=hlsl
#ifndef GI_FILTER_COMMON_HLSL
#define GI_FILTER_COMMON_HLSL
...
// GIDenoise.comp. Capsaicin reference: gi1.comp:4104–4154 (FilterGI) and
// gi_denoiser.hlsl:43–54 (GIDenoiser_GetBlurRadius). Two compilation
// units (`GIFilterHorizontal.comp`, `GIFilterVertical.comp`) define
...
#error "Define BLUR_DIRECTION (int2(1,0) for H, int2(0,1) for V) before including GIFilterCommon.hlsl"
#endif

// Capsaicin gi_denoiser.hlsl:26. Caps both the cap clamp in GIDenoise.comp
// (`kGIDenoiser_MaxBlurMask` there) and the spatial filter radius here.
...
// this frame by GIDenoise; V pass reads the H pass's scratch output
// (same shape).
[[vk::binding(2, 1)]] Texture2D<float4> in_GIInput : register(t2);
// Per-pixel blur mask written by GIDenoise.comp. Stored in [0, 1] for
// real surfaces (1 = max radius, 0 = no blur), with -1 as the sky
// sentinel. Multiply by `kGIDenoiser_MaxBlurMask` to recover Capsaicin's
// raw `blur_mask` value.
...
{
	int blur_radius = 0;
	float blur_mask = in_BlurMask.Load(int3(pos, 0)) * kGIDenoiser_MaxBlurMask;
