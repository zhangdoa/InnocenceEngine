...
// declared the four cache textures (in_SSRC, in_ProbePosition,
// in_ProbeNormal, in_ProbeMask) and the PerFrame_CB instance `g_Frame`
// before the #include. Two callers exist today: SSRCTemporal.comp consumes the
// full bilinear blend; SSRCFilter{Horizontal,Vertical}.comp use
...
{
	float3 c00 = in_SSRC[shReadCoord + uint2(0, 0)].rgb;
	float3 c11 = in_SSRC[shReadCoord + uint2(1, 0)].rgb;
	float3 c1_1 = in_SSRC[shReadCoord + uint2(2, 0)].rgb;
	float3 c10 = in_SSRC[shReadCoord + uint2(0, 1)].rgb;
	float3 c2_2 = in_SSRC[shReadCoord + uint2(1, 1)].rgb;
	float3 c2_1 = in_SSRC[shReadCoord + uint2(2, 1)].rgb;
	float3 c20 = in_SSRC[shReadCoord + uint2(0, 2)].rgb;
	float3 c21 = in_SSRC[shReadCoord + uint2(1, 2)].rgb;
	float3 c22 = in_SSRC[shReadCoord + uint2(2, 2)].rgb;
...
// relaxed-equal-weight backup ran. The temporal accumulator (SSRCTemporal.comp)
...
float4 SampleSSRC(float2 screenCoord, float3 pixelPos, float3 pixelNormal)
{
