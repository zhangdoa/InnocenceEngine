...
bool VXGILightPass::Setup(IServiceConfig *systemConfig)
{
	...
	m_IlluminanceVolume = l_graphicsService->AddTextureComponent("VoxelIlluminanceVolume/");
	...
	m_ShaderProgramComp = l_graphicsService->AddShaderProgramComponent("VXGILightPass/");

	m_ShaderProgramComp->m_ShaderFilePaths.m_CSPath = "voxelLightPass.comp/";

	m_RenderPassComp = l_graphicsService->AddRenderPassComponent("VXGILightPass/");
...
{	
	...
	l_graphicsService->Initialize(m_IlluminanceVolume);
...
bool VXGILightPass::PrepareCommandList(IRenderingContext* renderingContext)
{
	...
	// l_graphicsService->BindGPUResource(m_RenderPassComp, ShaderStage::Compute, l_PerFrameCBufferGPUBufferComp, 0);
	// l_graphicsService->BindGPUResource(m_RenderPassComp, ShaderStage::Compute, l_renderingContext->m_AlbedoVolume, 1);
	// l_graphicsService->BindGPUResource(m_RenderPassComp, ShaderStage::Compute, l_renderingContext->m_NormalVolume, 2);
	// l_graphicsService->BindGPUResource(m_RenderPassComp, ShaderStage::Compute, m_IlluminanceVolume, 3);
	...
	// l_graphicsService->UnbindGPUResource(m_RenderPassComp, ShaderStage::Compute, l_renderingContext->m_AlbedoVolume, 1);
	// l_graphicsService->UnbindGPUResource(m_RenderPassComp, ShaderStage::Compute, l_renderingContext->m_NormalVolume, 2);
	// l_graphicsService->UnbindGPUResource(m_RenderPassComp, ShaderStage::Compute, m_IlluminanceVolume, 3);
