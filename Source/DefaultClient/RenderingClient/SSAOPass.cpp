...
bool SSAOPass::Setup(IServiceConfig* systemConfig)
{
	...
	m_ShaderProgramComp = l_graphicsService->AddShaderProgramComponent("SSAONoisePass/");

	m_ShaderProgramComp->m_ShaderFilePaths.m_CSPath = "SSAONoisePass.comp/";

	m_RenderPassComp = l_graphicsService->AddRenderPassComponent("SSAONoisePass/");
	...
	m_CommandListComp_Graphics = l_graphicsService->AddCommandListComponent("SSAOPass/Graphics/");
	m_CommandListComp_Graphics->m_Type = GPUEngineType::Graphics;

	m_CommandListComp_Compute = l_graphicsService->AddCommandListComponent("SSAOPass/Compute/");
	m_CommandListComp_Compute->m_Type = GPUEngineType::Compute;

	m_SamplerComp = l_graphicsService->AddSamplerComponent("SSAONoisePass/");

	m_SamplerComp_RandomRot = l_graphicsService->AddSamplerComponent("SSAONoisePass_RandomRot/");
	...
	m_NoiseTexture = l_graphicsService->AddTextureComponent("SSAO_Noise/");
...
bool SSAOPass::Initialize()
{
	...
	l_graphicsService->Initialize(m_SamplerComp_RandomRot);
	l_graphicsService->Initialize(m_KernelGPUBuffer);
	l_graphicsService->Initialize(m_NoiseTexture, &m_Noise[0]);
...
bool SSAOPass::Terminate()
{
	...
	l_graphicsService->Delete(m_NoiseTexture);
	l_graphicsService->Delete(m_KernelGPUBuffer);
	l_graphicsService->Delete(m_SamplerComp_RandomRot);
...
bool SSAOPass::PrepareCommandList(IRenderingContext* renderingContext)
{
	...
	l_graphicsService->TryToTransitState(m_NoiseTexture, m_CommandListComp_Graphics, Accessibility::WriteOnly, Accessibility::ReadOnly);
	...
	l_graphicsService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, ShaderStage::Compute, m_KernelGPUBuffer, 1);
	l_graphicsService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, ShaderStage::Compute, OpaquePass::Get().GetRenderPassComp()->m_OutputMergerTarget->m_ColorOutputs[0], 2);
	l_graphicsService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, ShaderStage::Compute, OpaquePass::Get().GetRenderPassComp()->m_OutputMergerTarget->m_ColorOutputs[1], 3);
	l_graphicsService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, ShaderStage::Compute, m_NoiseTexture, 4);
	l_graphicsService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, ShaderStage::Compute, m_SamplerComp, 5);
	l_graphicsService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, ShaderStage::Compute, m_SamplerComp_RandomRot, 6);
	l_graphicsService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, ShaderStage::Compute, m_Result, 7);
...
bool SSAOPass::RenderTargetsCreationFunc()
{
	...
	m_Result = l_graphicsService->AddTextureComponent("SSAO_Result/");
