...
bool RadianceCacheReprojectionPass::Setup(IServiceConfig* systemConfig)
{
	...
	m_ShaderProgramComp = l_graphicsService->AddShaderProgramComponent("RadianceCacheReprojectionPass/");

	m_ShaderProgramComp->m_ShaderFilePaths.m_CSPath = "RadianceCacheReprojection.comp/";

	m_RenderPassComp = l_graphicsService->AddRenderPassComponent("RadianceCacheReprojectionPass/");
	...
	m_CommandListComp_Graphics = l_graphicsService->AddCommandListComponent("RadianceCacheReprojectionPass/Graphics/");
	m_CommandListComp_Graphics->m_Type = GPUEngineType::Graphics;

	m_CommandListComp_Compute = l_graphicsService->AddCommandListComponent("RadianceCacheReprojectionPass/Compute/");
...
bool RadianceCacheReprojectionPass::Terminate()
{
	...
	l_graphicsService->Delete(m_WorldProbeGrid);
	l_graphicsService->Delete(m_ProbePosition_Even);
	l_graphicsService->Delete(m_ProbePosition_Odd);
	l_graphicsService->Delete(m_ProbeNormal_Even);
	l_graphicsService->Delete(m_ProbeNormal_Odd);
	l_graphicsService->Delete(m_RadianceCache_Even);
	l_graphicsService->Delete(m_RadianceCache_Odd);
...
bool RadianceCacheReprojectionPass::PrepareCommandList(IRenderingContext* renderingContext)
{
	...
	l_graphicsService->Clear(m_CommandListComp_Graphics, l_writeTexture);
...
bool RadianceCacheReprojectionPass::RenderTargetsCreationFunc()
{
	...
	m_RadianceCache_Even = l_graphicsService->AddTextureComponent("Radiance Cache Result (Even)/");
	m_RadianceCache_Even->m_TextureDesc = l_RenderPassDesc.m_RenderTargetDesc;

	l_graphicsService->Initialize(m_RadianceCache_Even);

	if (m_RadianceCache_Odd)
		l_graphicsService->Delete(m_RadianceCache_Odd);

	m_RadianceCache_Odd = l_graphicsService->AddTextureComponent("Radiance Cache Result (Odd)/");
	m_RadianceCache_Odd->m_TextureDesc = m_RadianceCache_Even->m_TextureDesc;

	l_graphicsService->Initialize(m_RadianceCache_Odd);
	...
	m_ProbePosition_Odd = l_graphicsService->AddTextureComponent("Radiance Cache Probe Position (Odd)/");
	...
	l_graphicsService->Initialize(m_ProbePosition_Odd);

	if (m_ProbePosition_Even)
		l_graphicsService->Delete(m_ProbePosition_Even);

	m_ProbePosition_Even = l_graphicsService->AddTextureComponent("Radiance Cache Probe Position (Even)/");
	m_ProbePosition_Even->m_TextureDesc = m_ProbePosition_Odd->m_TextureDesc;

	l_graphicsService->Initialize(m_ProbePosition_Even);

	if (m_ProbeNormal_Odd)
		l_graphicsService->Delete(m_ProbeNormal_Odd);

	m_ProbeNormal_Odd = l_graphicsService->AddTextureComponent("Radiance Cache Probe Normal (Odd)/");
	m_ProbeNormal_Odd->m_TextureDesc = m_ProbePosition_Odd->m_TextureDesc;

	l_graphicsService->Initialize(m_ProbeNormal_Odd);

	if (m_ProbeNormal_Even)
		l_graphicsService->Delete(m_ProbeNormal_Even);

	m_ProbeNormal_Even = l_graphicsService->AddTextureComponent("Radiance Cache Probe Normal (Even)/");
	m_ProbeNormal_Even->m_TextureDesc = m_ProbePosition_Odd->m_TextureDesc;

	l_graphicsService->Initialize(m_ProbeNormal_Even);

	if (m_WorldProbeGrid)
		l_graphicsService->Delete(m_WorldProbeGrid);

	m_WorldProbeGrid = l_graphicsService->AddGPUBufferComponent("Radiance Cache World Probe Grid/");
	...
	l_graphicsService->Initialize(m_WorldProbeGrid);
