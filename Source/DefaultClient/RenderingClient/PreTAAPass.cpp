...
bool PreTAAPass::PrepareCommandList(IRenderingContext* renderingContext)
{
	...
	l_hwService->TryToTransitState(reinterpret_cast<TextureComponent*>(LightPass::Get().GetLuminanceResult()), m_CommandListComp_Graphics, Accessibility::WriteOnly, Accessibility::ReadOnly);
	l_hwService->TryToTransitState(reinterpret_cast<TextureComponent*>(SkyPass::Get().GetResult()), m_CommandListComp_Graphics, Accessibility::WriteOnly, Accessibility::ReadOnly);
	...
	l_hwService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, ShaderStage::Compute, LightPass::Get().GetLuminanceResult(), 0);
	l_hwService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, ShaderStage::Compute, SkyPass::Get().GetResult(), 1);
	l_hwService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, ShaderStage::Compute, m_Result, 2);
