...
bool PreTAAPass::PrepareCommandList(IRenderingContext* renderingContext)
{
	...
	l_fmService->TryToTransitState(reinterpret_cast<TextureComponent*>(LightPass::Get().GetLuminanceResult()), m_CommandListComp_Graphics, Accessibility::WriteOnly, Accessibility::ReadOnly);
	l_fmService->TryToTransitState(reinterpret_cast<TextureComponent*>(SkyPass::Get().GetResult()), m_CommandListComp_Graphics, Accessibility::WriteOnly, Accessibility::ReadOnly);
	...
	l_fmService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, ShaderStage::Compute, LightPass::Get().GetLuminanceResult(), 0);
	l_fmService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, ShaderStage::Compute, SkyPass::Get().GetResult(), 1);
	l_fmService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, ShaderStage::Compute, m_Result, 2);
