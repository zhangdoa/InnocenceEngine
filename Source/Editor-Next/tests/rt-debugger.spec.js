...
test('RT debugger lists passes and routes a chosen RT to the viewport', async () => {
  ...
  try {
    ...
    await window.waitForSelector('[data-test="rt-debugger-panel"]', {
      state: 'attached',
      timeout: 15000,
    });
    ...
    await window.waitForSelector('[data-test="rt-picker"]', { timeout: 15000 });

    const picker = window.locator('[data-test="rt-picker"]');
    await picker.click();
    ...
    await window.click('[data-test="rt-apply"]');
    await window.waitForSelector(`.n-message:has-text("Viewport now showing")`, {
      timeout: 10000,
    });

    await window.click('[data-test="rt-reset"]');
