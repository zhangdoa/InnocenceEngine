...
bool GIFilterHorizontalPass::Setup(IServiceConfig* systemConfig)
{
	m_ShaderProgramComp = g_Engine->Get<ShaderProgramResourceService>()->Add("GIFilterHorizontalPass");
	m_ShaderProgramComp->m_ShaderFilePaths.m_CSPath = "GIFilterHorizontal.comp";

	m_RenderPassComp = g_Engine->Get<RenderPassResourceService>()->Add("GIFilterHorizontalPass");

	auto l_RenderPassDesc = g_Engine->Get<RenderingConfigurationService>()->GetDefaultRenderPassDesc();
	l_RenderPassDesc.m_GPUEngineType = GPUEngineType::Compute;
	l_RenderPassDesc.m_RenderTargetCount = 0;
	l_RenderPassDesc.m_UseOutputMerger = false;
	l_RenderPassDesc.m_RenderTargetsInitializationFunc = std::bind(&GIFilterHorizontalPass::RenderTargetsCreationFunc, this);

	m_RenderPassComp->m_RenderPassDesc = l_RenderPassDesc;

	// 1 CB + 4 SRVs + 1 UAV — same shape as RadianceCacheFilter{H,V}Pass
	...
	m_CommandListComp_Graphics = g_Engine->Get<CommandListResourceService>()->Add("GIFilterHorizontalPass/Graphics");
	m_CommandListComp_Graphics->m_Type = GPUEngineType::Graphics;

	m_CommandListComp_Compute = g_Engine->Get<CommandListResourceService>()->Add("GIFilterHorizontalPass/Compute");
...
bool GIFilterHorizontalPass::Initialize()
{
...
bool GIFilterHorizontalPass::Terminate()
{
...
ObjectStatus GIFilterHorizontalPass::GetStatus() { return m_ObjectStatus; }

bool GIFilterHorizontalPass::PrepareCommandList(IRenderingContext* renderingContext)
{
	...
	auto l_GIInput = GIDenoisePass::Get().GetCurrentResult();
...
RenderPassComponent* GIFilterHorizontalPass::GetRenderPassComp() { return m_RenderPassComp; }
TextureComponent* GIFilterHorizontalPass::GetResult() { return m_Result; }

bool GIFilterHorizontalPass::RenderTargetsCreationFunc()
{
	if (m_Result)
		g_Engine->Get<TextureResourceService>()->Delete(m_Result);

	auto l_RenderPassDesc = g_Engine->Get<RenderingConfigurationService>()->GetDefaultRenderPassDesc();
	m_Result = g_Engine->Get<TextureResourceService>()->Add("GIFilterHorizontalPass Result");
