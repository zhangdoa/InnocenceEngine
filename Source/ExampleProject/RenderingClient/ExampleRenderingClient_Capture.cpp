...
namespace Inno
{
	...
	void ExampleRenderingClientImpl::HandleAutoCaptureTriggers()
	{
		...
		// Serialize-test mode sets totalFrames=1; skip the trigger so the readback never
		// runs before the present canvas is activated (fatal log path on unactivated texture).
