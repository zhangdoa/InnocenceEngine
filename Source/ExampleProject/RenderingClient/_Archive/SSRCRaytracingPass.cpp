...
bool SSRCRaytracingPass::Setup(IServiceConfig* systemConfig)
{
	auto l_fmService = g_Engine->Get<FrameManagementService>();

	m_SamplerComp = g_Engine->Get<SamplerResourceService>()->Add("SSRCRaytracingPass");

	m_ShaderProgramComp = g_Engine->Get<ShaderProgramResourceService>()->Add("SSRCRaytracingPass");

	m_ShaderProgramComp->m_ShaderFilePaths.m_RayGenPath = "SSRCRayGen.hlsl";
	m_ShaderProgramComp->m_ShaderFilePaths.m_ClosestHitPath = "SSRCClosestHit.hlsl";
	m_ShaderProgramComp->m_ShaderFilePaths.m_AnyHitPath = "SSRCAnyHit.hlsl";
	m_ShaderProgramComp->m_ShaderFilePaths.m_MissPath = "SSRCMiss.hlsl";

	m_RenderPassComp = g_Engine->Get<RenderPassResourceService>()->Add("SSRCRaytracingPass");
	...
	l_RenderPassDesc.m_RenderTargetsInitializationFunc = std::bind(&SSRCRaytracingPass::RenderTargetsCreationFunc, this);
	...
	m_CommandListComp_Graphics = g_Engine->Get<CommandListResourceService>()->Add("SSRCRaytracingPass/Graphics");
	m_CommandListComp_Graphics->m_Type = GPUEngineType::Graphics;

	m_CommandListComp_Compute = g_Engine->Get<CommandListResourceService>()->Add("SSRCRaytracingPass");
...
bool SSRCRaytracingPass::Initialize()
{
	auto l_fmService = g_Engine->Get<FrameManagementService>();
...
bool SSRCRaytracingPass::Terminate()
{
	auto l_fmService = g_Engine->Get<FrameManagementService>();
...
ObjectStatus SSRCRaytracingPass::GetStatus()
{
	return m_ObjectStatus;
}

bool SSRCRaytracingPass::PrepareCommandList(IRenderingContext* renderingContext)
{
	...
	auto l_result = SSRCReprojectionPass::Get().GetCurrentFrameResult();
	...
	l_fmService->TryToTransitState(SSRCReprojectionPass::Get().GetPreviousFrameResult(), m_CommandListComp_Graphics, Accessibility::WriteOnly, Accessibility::ReadOnly);
	...
	l_fmService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, m_ShaderStage, SSRCReprojectionPass::Get().GetPreviousFrameResult(), 7);
	l_fmService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, m_ShaderStage, l_result, 8);
	l_fmService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, m_ShaderStage, SSRCReprojectionPass::Get().GetCurrentProbePosition(), 9);
	l_fmService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, m_ShaderStage, SSRCReprojectionPass::Get().GetCurrentProbeNormal(), 10);
	l_fmService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, m_ShaderStage, SSRCReprojectionPass::Get().GetProbeMask(), 11);
	...
	auto dispatch_x = (l_result->m_TextureDesc.Width + SSRC::SPAWN_TILE_SIZE_X - 1u) / SSRC::SPAWN_TILE_SIZE_X;
	auto dispatch_y = (l_result->m_TextureDesc.Height + SSRC::SPAWN_TILE_SIZE_Y - 1u) / SSRC::SPAWN_TILE_SIZE_Y;

	auto l_hwService = g_Engine->Get<GraphicsHardwareService>();
	l_hwService->BeginGpuPass(m_CommandListComp_Compute, "SSRCRT", GPUEngineType::Compute);

	l_fmService->DispatchRays(m_RenderPassComp, m_CommandListComp_Compute, dispatch_x, dispatch_y, 1);

	l_hwService->EndGpuPass(m_CommandListComp_Compute, "SSRCRT", GPUEngineType::Compute);
...
RenderPassComponent* SSRCRaytracingPass::GetRenderPassComp()
{
	return m_RenderPassComp;
}

bool SSRCRaytracingPass::RenderTargetsCreationFunc()
{
