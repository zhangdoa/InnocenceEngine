...
// common/SSRCCommon.hlsl. Shaders own the canonical values; drift between this
...
namespace SSRC
{
    ...
    // in common/SSRCCommon.hlsl and TILE_SIZE in RayTracingTypes.hlsl.
    // Drives probe-grid extent (viewportSize / TILE_SIZE) and every dispatch
    // that runs at probe granularity (the four SSRC* passes plus
    // SSRCTemporal).
    ...
    // common/SSRCCommon.hlsl. Mirrors SH_TILE_SIZE in
    // RayTracingTypes.hlsl. The integration shader writes 9 cells per probe
    // in row-major order; the consumer (SampleSSRC) reads the same
    // 3×3 footprint.
