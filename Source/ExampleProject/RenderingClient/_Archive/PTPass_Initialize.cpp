...
bool PTPass::Initialize()
{
	...
	m_FrameCountCB = g_Engine->Get<GPUBufferResourceService>()->Add("PTFrameCountCB");
	...
	m_LightCountCB = g_Engine->Get<GPUBufferResourceService>()->Add("PTLightCountCB");
	m_LightCountCB->m_ElementCount      = 1;
	m_LightCountCB->m_ElementSize       = sizeof(PTLightCountData);
	...
	if constexpr (Inno::PTHashGridCache::ENABLED)
	{
		...
		// Site-3 in PTRayGen.hlsl as the direct-lobe estimator.
