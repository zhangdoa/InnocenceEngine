...
bool SSRCIntegrationPass::Setup(IServiceConfig* systemConfig)
{
	auto l_fmService = g_Engine->Get<FrameManagementService>();

	m_ShaderProgramComp = g_Engine->Get<ShaderProgramResourceService>()->Add("SSRCIntegrationPass");

	m_ShaderProgramComp->m_ShaderFilePaths.m_CSPath = "SSRCIntegration.comp";

	m_RenderPassComp = g_Engine->Get<RenderPassResourceService>()->Add("SSRCIntegrationPass");
	...
	l_RenderPassDesc.m_RenderTargetsInitializationFunc = std::bind(&SSRCIntegrationPass::RenderTargetsCreationFunc, this);
	...
	m_CommandListComp_Graphics = g_Engine->Get<CommandListResourceService>()->Add("SSRCIntegrationPass/Graphics");
	m_CommandListComp_Graphics->m_Type = GPUEngineType::Graphics;

	m_CommandListComp_Compute = g_Engine->Get<CommandListResourceService>()->Add("SSRCIntegrationPass/Compute");
...
bool SSRCIntegrationPass::Initialize()
{
	auto l_fmService = g_Engine->Get<FrameManagementService>();
...
bool SSRCIntegrationPass::Update()
{
...
bool SSRCIntegrationPass::Terminate()
{
	auto l_fmService = g_Engine->Get<FrameManagementService>();
...
ObjectStatus SSRCIntegrationPass::GetStatus()
{
	return m_ObjectStatus;
}

bool SSRCIntegrationPass::PrepareCommandList(IRenderingContext* renderingContext)
{
	if (m_RenderPassComp->m_ObjectStatus != ObjectStatus::Activated)
		return false;
	...
	auto l_readTexture = SSRCFilterVerticalPass::Get().GetResult();
	...
	auto dispatch_x = SSRC::TileCount(l_readTexture->m_TextureDesc.Width);
	auto dispatch_y = SSRC::TileCount(l_readTexture->m_TextureDesc.Height);
...
RenderPassComponent* SSRCIntegrationPass::GetRenderPassComp()
{
	return m_RenderPassComp;
}

GPUResourceComponent* SSRCIntegrationPass::GetResult()
{
	return m_Result;
}

bool SSRCIntegrationPass::RenderTargetsCreationFunc()
{
	auto l_fmService = g_Engine->Get<FrameManagementService>();
	...
	// Probe-grid extent: one screen probe per SSRC::TILE_SIZE pixels.
	// Per-probe SH atlas tile is SSRC::SH_TILE_SIZE × SH_TILE_SIZE
	...
	// the framebuffer because SSRCIntegration.comp writes (and
	// SampleSSRC reads) at probeIndex * 3.
	const auto l_probeGridWidth = SSRC::TileCount(m_Result->m_TextureDesc.Width);
	const auto l_probeGridHeight = SSRC::TileCount(m_Result->m_TextureDesc.Height);
	m_Result->m_TextureDesc.Width = l_probeGridWidth * SSRC::SH_TILE_SIZE;
	m_Result->m_TextureDesc.Height = l_probeGridHeight * SSRC::SH_TILE_SIZE;
