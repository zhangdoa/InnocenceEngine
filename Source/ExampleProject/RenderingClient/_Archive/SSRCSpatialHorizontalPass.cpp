...
bool SSRCSpatialHorizontalPass::Setup(IServiceConfig* systemConfig)
{
	m_ShaderProgramComp = g_Engine->Get<ShaderProgramResourceService>()->Add("SSRCSpatialHorizontalPass");
	m_ShaderProgramComp->m_ShaderFilePaths.m_CSPath = "SSRCSpatialHorizontal.comp";

	m_RenderPassComp = g_Engine->Get<RenderPassResourceService>()->Add("SSRCSpatialHorizontalPass");

	auto l_RenderPassDesc = g_Engine->Get<RenderingConfigurationService>()->GetDefaultRenderPassDesc();
	l_RenderPassDesc.m_GPUEngineType = GPUEngineType::Compute;
	l_RenderPassDesc.m_RenderTargetCount = 0;
	l_RenderPassDesc.m_UseOutputMerger = false;
	l_RenderPassDesc.m_RenderTargetsInitializationFunc = std::bind(&SSRCSpatialHorizontalPass::RenderTargetsCreationFunc, this);

	m_RenderPassComp->m_RenderPassDesc = l_RenderPassDesc;

	// 1 CB + 4 SRVs + 1 UAV — same shape as SSRCFilter{H,V}Pass
	// since both implement Capsaicin's separable bilateral blur on
	// different inputs (probe space vs. screen space).
	...
	// t0 - G-buffer world position (depth surrogate + sky flag)
	...
	// t1 - G-buffer world normal (bilateral normal weight)
	...
	// t2 - GI history (sample-count-weighted, this frame's denoiser output)
	...
	// u0 - horizontal scratch (rgb·N, N) for the vertical pass
	...
	m_CommandListComp_Graphics = g_Engine->Get<CommandListResourceService>()->Add("SSRCSpatialHorizontalPass/Graphics");
	m_CommandListComp_Graphics->m_Type = GPUEngineType::Graphics;

	m_CommandListComp_Compute = g_Engine->Get<CommandListResourceService>()->Add("SSRCSpatialHorizontalPass/Compute");
...
bool SSRCSpatialHorizontalPass::Initialize()
{
...
bool SSRCSpatialHorizontalPass::Terminate()
{
...
ObjectStatus SSRCSpatialHorizontalPass::GetStatus() { return m_ObjectStatus; }

bool SSRCSpatialHorizontalPass::PrepareCommandList(IRenderingContext* renderingContext)
{
	...
	auto l_GIInput = SSRCTemporalPass::Get().GetCurrentResult();
	...
	l_fmService->TryToTransitState(l_GIInput, m_CommandListComp_Graphics, Accessibility::WriteOnly, Accessibility::ReadOnly);
	...
	l_fmService->BindGPUResource(m_RenderPassComp, m_CommandListComp_Compute, ShaderStage::Compute, l_GIInput, 3);
...
RenderPassComponent* SSRCSpatialHorizontalPass::GetRenderPassComp() { return m_RenderPassComp; }
TextureComponent* SSRCSpatialHorizontalPass::GetResult() { return m_Result; }

bool SSRCSpatialHorizontalPass::RenderTargetsCreationFunc()
{
	if (m_Result)
		g_Engine->Get<TextureResourceService>()->Delete(m_Result);

	auto l_RenderPassDesc = g_Engine->Get<RenderingConfigurationService>()->GetDefaultRenderPassDesc();
	m_Result = g_Engine->Get<TextureResourceService>()->Add("SSRCSpatialHorizontalPass Result");
