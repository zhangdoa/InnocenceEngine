...
bool SSRCReprojectionPass::Setup(IServiceConfig* systemConfig)
{

	m_ShaderProgramComp = g_Engine->Get<ShaderProgramResourceService>()->Add("SSRCReprojectionPass");

	m_ShaderProgramComp->m_ShaderFilePaths.m_CSPath = "SSRCReprojection.comp";

	m_RenderPassComp = g_Engine->Get<RenderPassResourceService>()->Add("SSRCReprojectionPass");
	...
	l_RenderPassDesc.m_RenderTargetsInitializationFunc = std::bind(&SSRCReprojectionPass::RenderTargetsCreationFunc, this);
	...
	m_CommandListComp_Graphics = g_Engine->Get<CommandListResourceService>()->Add("SSRCReprojectionPass/Graphics");
	m_CommandListComp_Graphics->m_Type = GPUEngineType::Graphics;

	m_CommandListComp_Compute = g_Engine->Get<CommandListResourceService>()->Add("SSRCReprojectionPass/Compute");
...
bool SSRCReprojectionPass::RenderTargetsCreationFunc()
{
	...
	m_SSRC_Even = g_Engine->Get<TextureResourceService>()->Add("Radiance Cache Result (Even)");
	m_SSRC_Even->m_TextureDesc = l_RenderPassDesc.m_RenderTargetDesc;

	g_Engine->Get<TextureResourceService>()->Initialize(m_SSRC_Even);

	if (m_SSRC_Odd)
		g_Engine->Get<TextureResourceService>()->Delete(m_SSRC_Odd);

	m_SSRC_Odd = g_Engine->Get<TextureResourceService>()->Add("Radiance Cache Result (Odd)");
	m_SSRC_Odd->m_TextureDesc = m_SSRC_Even->m_TextureDesc;

	g_Engine->Get<TextureResourceService>()->Initialize(m_SSRC_Odd);

	if (m_ProbePosition_Odd)
		g_Engine->Get<TextureResourceService>()->Delete(m_ProbePosition_Odd);

	auto l_probeTextureWidth = SSRC::TileCount(l_RenderPassDesc.m_RenderTargetDesc.Width);
	auto l_probeTextureHeight = SSRC::TileCount(l_RenderPassDesc.m_RenderTargetDesc.Height);
