...
bool SSRCFilterHorizontalPass::Setup(IServiceConfig* systemConfig)
{
	auto l_fmService = g_Engine->Get<FrameManagementService>();

	m_ShaderProgramComp = g_Engine->Get<ShaderProgramResourceService>()->Add("SSRCFilterHorizontalPass");
	m_ShaderProgramComp->m_ShaderFilePaths.m_CSPath = "SSRCFilterHorizontal.comp";

	m_RenderPassComp = g_Engine->Get<RenderPassResourceService>()->Add("SSRCFilterHorizontalPass");
	
	auto l_RenderPassDesc = g_Engine->Get<RenderingConfigurationService>()->GetDefaultRenderPassDesc();
	l_RenderPassDesc.m_GPUEngineType = GPUEngineType::Compute;
	l_RenderPassDesc.m_RenderTargetCount = 0;
	l_RenderPassDesc.m_UseOutputMerger = false;
	l_RenderPassDesc.m_RenderTargetsInitializationFunc = std::bind(&SSRCFilterHorizontalPass::RenderTargetsCreationFunc, this);
	...
	m_CommandListComp_Graphics = g_Engine->Get<CommandListResourceService>()->Add("SSRCFilterHorizontalPass/Graphics");
	m_CommandListComp_Graphics->m_Type = GPUEngineType::Graphics;

	m_CommandListComp_Compute = g_Engine->Get<CommandListResourceService>()->Add("SSRCFilterHorizontalPass/Compute");
...
bool SSRCFilterHorizontalPass::Initialize()
{
	auto l_fmService = g_Engine->Get<FrameManagementService>();
...
bool SSRCFilterHorizontalPass::Terminate()
{
	auto l_fmService = g_Engine->Get<FrameManagementService>();
...
ObjectStatus SSRCFilterHorizontalPass::GetStatus()
{
	return m_ObjectStatus;
}

bool SSRCFilterHorizontalPass::PrepareCommandList(IRenderingContext* renderingContext)
{
	...
	auto l_raytracingResult = SSRCReprojectionPass::Get().GetCurrentFrameResult();
	...
	auto dispatch_x = SSRC::TileCount(l_raytracingResult->m_TextureDesc.Width);
	auto dispatch_y = SSRC::TileCount(l_raytracingResult->m_TextureDesc.Height);
...
RenderPassComponent* SSRCFilterHorizontalPass::GetRenderPassComp()
{
	return m_RenderPassComp;
}

TextureComponent* SSRCFilterHorizontalPass::GetResult()
{
	return m_Result;
}

bool SSRCFilterHorizontalPass::RenderTargetsCreationFunc()
{
	auto l_fmService = g_Engine->Get<FrameManagementService>();
	auto l_raytracingResult = SSRCReprojectionPass::Get().GetCurrentFrameResult();
	...
	m_Result = g_Engine->Get<TextureResourceService>()->Add("SSRCFilterHorizontalPass_Result");
