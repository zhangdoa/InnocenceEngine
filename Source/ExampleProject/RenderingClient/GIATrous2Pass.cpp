...
bool GIATrous2Pass::Setup(IServiceConfig* systemConfig)
{
	m_ShaderProgramComp = g_Engine->Get<ShaderProgramResourceService>()->Add("GIATrous2Pass");
	m_ShaderProgramComp->m_ShaderFilePaths.m_CSPath = "GIATrousStride2.comp";

	m_RenderPassComp = g_Engine->Get<RenderPassResourceService>()->Add("GIATrous2Pass");

	auto l_RenderPassDesc = g_Engine->Get<RenderingConfigurationService>()->GetDefaultRenderPassDesc();
	l_RenderPassDesc.m_GPUEngineType = GPUEngineType::Compute;
	l_RenderPassDesc.m_RenderTargetCount = 0;
	l_RenderPassDesc.m_UseOutputMerger = false;
	l_RenderPassDesc.m_RenderTargetsInitializationFunc = std::bind(&GIATrous2Pass::RenderTargetsCreationFunc, this);
	...
	m_CommandListComp_Graphics = g_Engine->Get<CommandListResourceService>()->Add("GIATrous2Pass/Graphics");
	m_CommandListComp_Graphics->m_Type = GPUEngineType::Graphics;

	m_CommandListComp_Compute = g_Engine->Get<CommandListResourceService>()->Add("GIATrous2Pass/Compute");
...
ObjectStatus GIATrous2Pass::GetStatus() { return m_ObjectStatus; }

bool GIATrous2Pass::PrepareCommandList(IRenderingContext* renderingContext)
{
	...
	auto l_input = GIATrous1Pass::Get().GetResult();
...
RenderPassComponent* GIATrous2Pass::GetRenderPassComp() { return m_RenderPassComp; }
TextureComponent* GIATrous2Pass::GetResult() { return m_Result; }

bool GIATrous2Pass::RenderTargetsCreationFunc()
{
	if (m_Result)
		g_Engine->Get<TextureResourceService>()->Delete(m_Result);

	auto l_RenderPassDesc = g_Engine->Get<RenderingConfigurationService>()->GetDefaultRenderPassDesc();
	m_Result = g_Engine->Get<TextureResourceService>()->Add("GIATrous2Pass Result");
