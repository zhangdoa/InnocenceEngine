...
bool GIATrous4Pass::Setup(IServiceConfig* systemConfig)
{
	m_ShaderProgramComp = g_Engine->Get<ShaderProgramResourceService>()->Add("GIATrous4Pass");
	m_ShaderProgramComp->m_ShaderFilePaths.m_CSPath = "GIATrousStride4.comp";

	m_RenderPassComp = g_Engine->Get<RenderPassResourceService>()->Add("GIATrous4Pass");

	auto l_RenderPassDesc = g_Engine->Get<RenderingConfigurationService>()->GetDefaultRenderPassDesc();
	l_RenderPassDesc.m_GPUEngineType = GPUEngineType::Compute;
	l_RenderPassDesc.m_RenderTargetCount = 0;
	l_RenderPassDesc.m_UseOutputMerger = false;
	l_RenderPassDesc.m_RenderTargetsInitializationFunc = std::bind(&GIATrous4Pass::RenderTargetsCreationFunc, this);
	...
	m_CommandListComp_Graphics = g_Engine->Get<CommandListResourceService>()->Add("GIATrous4Pass/Graphics");
	m_CommandListComp_Graphics->m_Type = GPUEngineType::Graphics;

	m_CommandListComp_Compute = g_Engine->Get<CommandListResourceService>()->Add("GIATrous4Pass/Compute");
...
ObjectStatus GIATrous4Pass::GetStatus() { return m_ObjectStatus; }

bool GIATrous4Pass::PrepareCommandList(IRenderingContext* renderingContext)
{
	...
	auto l_input = GIATrous2Pass::Get().GetResult();
...
RenderPassComponent* GIATrous4Pass::GetRenderPassComp() { return m_RenderPassComp; }
TextureComponent* GIATrous4Pass::GetResult() { return m_Result; }

bool GIATrous4Pass::RenderTargetsCreationFunc()
{
	if (m_Result)
		g_Engine->Get<TextureResourceService>()->Delete(m_Result);

	auto l_RenderPassDesc = g_Engine->Get<RenderingConfigurationService>()->GetDefaultRenderPassDesc();
	m_Result = g_Engine->Get<TextureResourceService>()->Add("GIATrous4Pass Result");
