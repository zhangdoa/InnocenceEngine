...
ID3D10Blob* DX11RenderingBackendNS::loadShaderBuffer(ShaderType shaderType, const std::wstring & shaderFilePath)
{
	...
	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_VERBOSE, "DX11RenderingBackend: innoShader: " + l_shaderName + " Shader has been compiled.");
...
DX11ConstantBuffer DX11RenderingBackendNS::createConstantBuffer(size_t elementSize, size_t elementCount, const std::string& name)
{
	...
	if (FAILED(l_hResult))
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create constant buffer pointer!");
	}
	...
	if (FAILED(l_hResult))
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't set name for constant buffer!");
	}
...
bool DX11RenderingBackendNS::createStructuredBuffer(void* initialData, DX11StructuredBuffer& arg, const std::string& name)
{
	if (arg.m_StructuredBufferDesc.ByteWidth > 0)
	{
		...
		if (FAILED(l_hResult))
		{
			g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create structured buffer pointer!");
			return false;
		}
		...
		if (FAILED(l_hResult))
		{
			g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't set name for structured buffer!");
		}
		...
		if (FAILED(l_hResult))
		{
			g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create SRV!");
			return false;
		}
		...
		if (FAILED(l_hResult))
		{
			g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create UAV!");
			return false;
		}
	...
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: structured buffer byte width is 0!");
		return false;
	}
}
...
bool DX11RenderingBackendNS::createVertexShader(ID3D10Blob* shaderBuffer, ID3D11VertexShader** vertexShader)
{
	...
	if (FAILED(result))
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create vertex shader!");
		return false;
	}
...
bool DX11RenderingBackendNS::createInputLayout(ID3D10Blob* shaderBuffer, ID3D11InputLayout** inputLayout)
{
	...
	if (FAILED(result))
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create vertex shader layout!");
		return false;
	}
...
bool DX11RenderingBackendNS::initializePixelShader(DX11ShaderProgramComponent* rhs, const std::wstring& PSShaderPath)
{
	...
	if (FAILED(result))
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create texture sampler state!");
		return false;
	}
...
bool DX11RenderingBackendNS::createPixelShader(ID3D10Blob* shaderBuffer, ID3D11PixelShader** pixelShader)
{
	...
	if (FAILED(result))
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create pixel shader!");
		return false;
	}
...
bool DX11RenderingBackendNS::createComputeShader(ID3D10Blob* shaderBuffer, ID3D11ComputeShader** computeShader)
{
	...
	if (FAILED(result))
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create compute shader!");
		return false;
	}
...
void DX11RenderingBackendNS::OutputShaderErrorMessage(ID3D10Blob * errorMessage, HWND hwnd, const std::string & shaderFilename)
{
	...
	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: innoShader: " + shaderFilename + " compile error: " + errorSStream.str() + "\n -- --------------------------------------------------- -- ");
}
...
bool DX11RenderingBackendNS::reserveRenderTargets(DX11RenderPassComponent* DXRPC)
{
	...
	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_SUCCESS, "DX11RenderingBackend: " + std::string(DXRPC->m_name.c_str()) + " render targets have been allocated.");
...
bool DX11RenderingBackendNS::createRenderTargets(DX11RenderPassComponent* DXRPC)
{
	...
	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_SUCCESS, "DX11RenderingBackend: " + std::string(DXRPC->m_name.c_str()) + " render targets have been created.");
...
bool DX11RenderingBackendNS::createRTV(DX11RenderPassComponent* DXRPC)
{
	...
	for (unsigned int i = 0; i < DXRPC->m_renderPassDesc.RTNumber; i++)
	{
		...
		if (FAILED(l_result))
		{
			g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create RTV for " + std::string(DXRPC->m_name.c_str()) + "!");
			return false;
		}
	}

	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_SUCCESS, "DX11RenderingBackend: " + std::string(DXRPC->m_name.c_str()) + " RTV has been created.");
...
bool DX11RenderingBackendNS::createDSV(DX11RenderPassComponent* DXRPC)
{
	...
	if (FAILED(l_result))
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create the DSV for " + std::string(DXRPC->m_name.c_str()) + "!");
		return false;
	}

	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_SUCCESS, "DX11RenderingBackend: " + std::string(DXRPC->m_name.c_str()) + " DSV has been created.");
...
bool DX11RenderingBackendNS::setupPipeline(DX11RenderPassComponent* DXRPC)
{
	if (DXRPC->m_renderPassDesc.useDepthAttachment)
	{
		...
		if (FAILED(l_result))
		{
			g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create the depth stencil state for for " + std::string(DXRPC->m_name.c_str()) + "!");
			return false;
		}
	}
	...
	if (FAILED(l_result))
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create the rasterizer state for " + std::string(DXRPC->m_name.c_str()) + "!");
		return false;
	}

	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_SUCCESS, "DX11RenderingBackend: " + std::string(DXRPC->m_name.c_str()) + " pipeline state has been setup.");
...
bool DX11RenderingBackendNS::initializeDX11TextureDataComponent(DX11TextureDataComponent * rhs)
{
	...
	else
	{
		...
		{
			g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_WARNING, "DX11RenderingBackend: try to generate DX11TextureDataComponent for TextureUsageType::INVISIBLE type!");
			return false;
		}
		...
		else
		{
			...
			{
				g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_WARNING, "DX11RenderingBackend: try to generate DX11TextureDataComponent without raw data!");
				return false;
			}
		}
	}
...
bool DX11RenderingBackendNS::submitGPUData(DX11MeshDataComponent * rhs)
{
	...
	if (FAILED(result))
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create vertex Buffer!");
		return false;
	}

	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_VERBOSE, "DX11RenderingBackend: vertex Buffer: " + InnoUtility::pointerToString(rhs->m_vertexBuffer) + " is initialized.");
	...
	if (FAILED(result))
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create index Buffer!");
		return false;
	}

	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_VERBOSE, "DX11RenderingBackend: index Buffer: " + InnoUtility::pointerToString(rhs->m_indexBuffer) + " is initialized.");
...
bool DX11RenderingBackendNS::submitGPUData(DX11TextureDataComponent * rhs)
{
	...
	if (FAILED(hResult))
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create texture!");
		return false;
	}

	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_VERBOSE, "DX11RenderingBackend: Texture: " + InnoUtility::pointerToString(rhs->m_texture) + " is initialized.");
	...
	if (FAILED(hResult))
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create SRV for texture!");
		return false;
	}
	...
	if (rhs->m_textureDataDesc.usageType == TextureUsageType::RAW_IMAGE)
	{
		...
		if (FAILED(hResult))
		{
			g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't create UAV for texture!");
			return false;
		}
	}
	...
	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_VERBOSE, "DX11RenderingBackend: SRV: " + InnoUtility::pointerToString(rhs->m_SRV) + " is initialized.");
...
void DX11RenderingBackendNS::bindTextureForWrite(ShaderType shaderType, unsigned int startSlot, DX11TextureDataComponent* DXTDC)
{
	...
	{
	case ShaderType::VERTEX:
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: Only compute shader support write to texture!");
		break;
	case ShaderType::GEOMETRY:
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: Only compute shader support write to texture!");
		break;
...
void DX11RenderingBackendNS::updateConstantBufferImpl(const DX11ConstantBuffer& ConstantBuffer, size_t size, const void* ConstantBufferValue)
{
	...
	if (FAILED(l_result))
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: can't lock the shader buffer!");
		return;
	}
...
void DX11RenderingBackendNS::bindStructuredBufferForWrite(ShaderType shaderType, unsigned int startSlot, const DX11StructuredBuffer& StructuredBuffer)
{
	...
	{
	case ShaderType::VERTEX:
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: Only compute shader support write to structured buffer!");
		break;
	case ShaderType::GEOMETRY:
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "DX11RenderingBackend: Only compute shader support write to structured buffer!");
		break;
