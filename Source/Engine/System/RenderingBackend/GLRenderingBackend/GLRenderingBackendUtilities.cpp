...
void GLRenderingBackendNS::generateFBO(GLRenderPassComponent* GLRPC)
{
	...
	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_SUCCESS, "GLRenderingBackend: " + std::string(GLRPC->m_name.c_str()) + " FBO has been generated.");
}
...
void GLRenderingBackendNS::generateRBO(GLRenderPassComponent* GLRPC)
{
	...
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "GLRenderingBackend: " + std::string(GLRPC->m_name.c_str()) + " Framebuffer is not completed: " + std::to_string(l_result));
	}
	else
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_SUCCESS, "GLRenderingBackend: " + std::string(GLRPC->m_name.c_str()) + " RBO has been generated.");
	}
}
...
void GLRenderingBackendNS::attachRenderTargets(GLRenderPassComponent* GLRPC, TextureDataDesc RTDesc, unsigned int colorAttachmentIndex)
{
	...
	else if (RTDesc.samplerType == TextureSamplerType::SAMPLER_3D)
	{
		if (RTDesc.pixelDataFormat == TexturePixelDataFormat::DEPTH_COMPONENT)
		{
			g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_WARNING, "GLRenderingBackend: 3D depth only render target is not supported now");
		}
		else
		{
	...
	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_SUCCESS, "GLRenderingBackend: " + std::string(GLRPC->m_name.c_str()) + " render target has been attached.");
}
...
bool GLRenderingBackendNS::initializeGLTextureDataComponent(GLTextureDataComponent * rhs)
{
	...
	else
	{
		...
		{
			g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_WARNING, "GLRenderingBackend: try to generate GLTextureDataComponent for TextureUsageType::INVISIBLE type!");
			return false;
		}
		...
		else
		{
			...
			{
				g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_WARNING, "GLRenderingBackend: try to generate GLTextureDataComponent without raw data!");
				return false;
			}
		}
	}
...
void GLRenderingBackendNS::addShader(GLuint& shaderProgram, GLuint& shaderID, GLuint shaderType, const ShaderFilePath& shaderFilePath)
{
	...
	if (shaderID == 0)
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "GLRenderingBackend: Shader creation failed! Memory location is invaild when add shader!");
	...
	if (shaderFilePath.find(".spv"))
	{
		...
		{
			g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "GLRenderingBackend: " + std::string(shaderFilePath.c_str()) + " content is empty!");
			return;
		}
	...
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "GLRenderingBackend: " + std::string(shaderFilePath.c_str()) + " compile failed!");
		glGetShaderiv(shaderID, GL_SHADER_SOURCE_LENGTH, &l_shaderFileLength);
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "GLRenderingBackend: " + std::string(shaderFilePath.c_str()) + " file length is: " + std::to_string(l_shaderFileLength));
		...
		if (l_infoLogLength > 0)
		{
			...
			g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "GLRenderingBackend: " + std::string(shaderFilePath.c_str()) + " compile error: " + &l_shaderErrorMessage[0] + "\n -- --------------------------------------------------- -- ");
		}
		else
		{
			g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "GLRenderingBackend: " + std::string(shaderFilePath.c_str()) + " compile error: no info log provided!");
		}

		return;
	}

	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_SUCCESS, "GLRenderingBackend: " + std::string(shaderFilePath.c_str()) + " has been compiled.");
	...
	if (!l_validationResult)
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "GLRenderingBackend: " + std::string(shaderFilePath.c_str()) + " link failed!");
		...
		if (l_infoLogLength > 0) {
			...
			g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "GLRenderingBackend: " + std::string(shaderFilePath.c_str()) + " link error: " + &l_shaderErrorMessage[0] + "\n -- --------------------------------------------------- -- ");
		}
		else
		{
			g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "GLRenderingBackend: " + std::string(shaderFilePath.c_str()) + " link error: no info log provided!");
		}

		return;
	}

	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_SUCCESS, "GLRenderingBackend: " + std::string(shaderFilePath.c_str()) + " has been linked.");
}
...
void GLRenderingBackendNS::generateTO(GLuint& TO, GLTextureDataDesc desc, const void* textureData)
{
	...
	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_VERBOSE, "GLRenderingBackend: Texture object " + std::to_string(TO) + " is created.");
}
...
bool GLRenderingBackendNS::submitGPUData(GLMeshDataComponent * rhs)
{
	...
	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_VERBOSE, "GLRenderingBackend: Vertex Array Object " + std::to_string(rhs->m_VAO) + " is initialized.");

	glBufferData(GL_ARRAY_BUFFER, l_verticesBuffer.size() * sizeof(float), &l_verticesBuffer[0], GL_STATIC_DRAW);

	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_VERBOSE, "GLRenderingBackend: Vertex Buffer Object " + std::to_string(rhs->m_VBO) + " is initialized.");

	glBufferData(GL_ELEMENT_ARRAY_BUFFER, rhs->m_indices.size() * sizeof(unsigned int), &rhs->m_indices[0], GL_STATIC_DRAW);

	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_VERBOSE, "GLRenderingBackend: Index Buffer Object " + std::to_string(rhs->m_IBO) + " is initialized.");
...
bool GLRenderingBackendNS::submitGPUData(GLTextureDataComponent * rhs)
{
	...
	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_VERBOSE, "GLRenderingBackend: GLTDC " + InnoUtility::pointerToString(rhs) + " is initialized.");
...
bool GLRenderingBackendNS::deleteShaderProgram(GLShaderProgramComponent* rhs)
{
	...
	g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_SUCCESS, "GLRenderingBackend: shader deleted.");
...
GLuint GLRenderingBackendNS::getUniformLocation(GLuint shaderProgram, const std::string & uniformName)
{
	...
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "GLRenderingBackend: Uniform lost: " + uniformName);
		return -1;
	}
