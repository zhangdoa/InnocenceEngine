...
bool ImGuiWrapper::initialize()
{
	if (ImGuiWrapperNS::m_isParity)
	{
		...
		ImGuiWrapperNS::m_renderingConfig = g_pCoreSystem->getRenderingFrontend()->getRenderingConfig();
	}
...
void ImGuiWrapperNS::showApplicationProfiler()
{
	...
	{
		g_pCoreSystem->getRenderingBackend()->reloadShader(RenderPassType(l_reloadShaderItem));
	}
	...
	{
		g_pCoreSystem->getRenderingBackend()->bakeGI();
	}
	...
	g_pCoreSystem->getRenderingFrontend()->setRenderingConfig(m_renderingConfig);
}
...
void ImGuiWrapperNS::showWorldExplorer()
{
	...
	auto l_entities = g_pCoreSystem->getGameSystem()->getEntities();
