...
ModelMap InnoFileSystemNS::AssetLoader::loadModel(const std::string & fileName)
{
	...
	if (l_extension == ".InnoModel")
	{
		...
		{
			g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_DEV_VERBOSE, "FileSystem: AssetLoader: " + fileName + " has been already loaded.");
...
TextureDataComponent* InnoFileSystemNS::AssetLoader::loadTextureFromDisk(const std::string & fileName)
{
	...
	if (l_rawData)
	{
		auto l_TDC = g_pCoreSystem->getRenderingFrontend()->addTextureDataComponent();
		...
		g_pCoreSystem->getRenderingBackend()->registerUninitializedTextureDataComponent(l_TDC);

		return l_TDC;
	}
	else
	{
		g_pCoreSystem->getLogSystem()->printLog(LogType::INNO_ERROR, "FileSystem: AssetLoader: STB_Image: Failed to load texture: " + l_fullPath);
