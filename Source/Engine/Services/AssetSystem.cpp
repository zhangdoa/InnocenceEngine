...
{
	g_Engine->Get<ComponentManager>()->RegisterType<VisibleComponent>(32768, this);
...
bool AssetSystem::LoadAssetsForComponents(bool AsyncLoad)
{
	...
	for (auto i : l_visibleComponents)
	{
		if (i->m_meshSource != MeshSource::Customized)
		{
			if (AsyncLoad)
			{
				auto l_loadModelTask = g_Engine->Get<TaskScheduler>()->Submit("AssignProceduralModelTask", 4, f_AssignProceduralModelTask, i, true);
				auto l_PDCTask = g_Engine->Get<TaskScheduler>()->Submit("PDCTask", 4, f_GeneratePDCTask, i);
			}
			else
			{
