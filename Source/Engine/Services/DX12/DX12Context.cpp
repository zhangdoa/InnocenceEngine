...
ComPtr<ID3D12Resource> DX12Context::CreateReadBackHeapBuffer(UINT64 size, const char* name)
{
	...
	if (FAILED(l_HResult))
	{
		...
		if (l_removeReason != S_OK)
		{
			...
			auto l_dredHR = m_device->QueryInterface(IID_PPV_ARGS(&l_pDred));
			Log(Warning, "DRED: QueryInterface HR=", static_cast<int32_t>(l_dredHR));
			if (SUCCEEDED(l_dredHR))
			{
				D3D12_DRED_AUTO_BREADCRUMBS_OUTPUT1 l_breadcrumbs = {};
				auto l_bcHR = l_pDred->GetAutoBreadcrumbsOutput1(&l_breadcrumbs);
				Log(Warning, "DRED: GetAutoBreadcrumbsOutput1 HR=", static_cast<int32_t>(l_bcHR),
					" pHead=", l_breadcrumbs.pHeadAutoBreadcrumbNode ? "valid" : "null");
				if (SUCCEEDED(l_bcHR))
				{
					...
					while (l_node)
					{
						...
						Log(Warning, "DRED[", nodeIdx, "] CL='",
							...
							" pLastBreadcrumbValue=", l_node->pLastBreadcrumbValue ? "valid" : "null");
					...
					if (nodeIdx == 0) Log(Warning, "DRED: no breadcrumb nodes found.");
				}
