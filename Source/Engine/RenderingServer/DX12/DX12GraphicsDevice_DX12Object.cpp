...
DX12SRV DX12GraphicsDevice::CreateSRV(D3D12_SHADER_RESOURCE_VIEW_DESC desc, ComPtr<ID3D12Resource> resourceHandle)
{
...
DX12UAV DX12GraphicsDevice::CreateUAV(D3D12_UNORDERED_ACCESS_VIEW_DESC desc, ComPtr<ID3D12Resource> resourceHandle, bool isAtomicCounter)
{
	auto l_CSUDescSize = m_device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
...
ComPtr<ID3D12Resource> DX12GraphicsDevice::CreateUploadHeapBuffer(D3D12_RESOURCE_DESC* resourceDesc, const char* name)
{
...
ComPtr<ID3D12Resource> DX12GraphicsDevice::CreateDefaultHeapBuffer(D3D12_RESOURCE_DESC* resourceDesc, D3D12_CLEAR_VALUE* clearValue, const char* name)
{
...
ComPtr<ID3D12Resource> DX12GraphicsDevice::CreateReadBackHeapBuffer(UINT64 size, const char* name)
{
...
ComPtr<ID3D12CommandQueue> DX12GraphicsDevice::CreateCommandQueue(D3D12_COMMAND_QUEUE_DESC* commandQueueDesc, const wchar_t* name)
{
...
ComPtr<ID3D12CommandAllocator> DX12GraphicsDevice::CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE commandListType, const wchar_t* name)
{
...
ComPtr<ID3D12GraphicsCommandList> DX12GraphicsDevice::CreateCommandList(D3D12_COMMAND_LIST_TYPE commandListType, ComPtr<ID3D12CommandAllocator> commandAllocator, const wchar_t* name)
{
...
ComPtr<ID3D12GraphicsCommandList> DX12GraphicsDevice::CreateTemporaryCommandList(D3D12_COMMAND_LIST_TYPE commandListType, ComPtr<ID3D12CommandAllocator> commandAllocator)
{
...
ComPtr<ID3D12DescriptorHeap> DX12GraphicsDevice::CreateDescriptorHeap(D3D12_DESCRIPTOR_HEAP_DESC desc, const wchar_t* name)
{
...
bool DX12GraphicsDevice::ExecuteCommandListAndWait(ComPtr<ID3D12GraphicsCommandList> commandList, ComPtr<ID3D12CommandQueue> commandQueue)
{
