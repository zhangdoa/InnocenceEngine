		...
		*/
		...
		inline auto transformAABBSpace(const TAABB<T>& rhs, TMat4<T> Tm) -> TAABB<T>
		{
			...
			for (auto& i : l_vertices)
			{
				...
				i.m_pos = InnoMath::mul(Vec4(i.m_pos, 1.0f), Tm).xyz();
#endif
#ifdef USE_ROW_MAJOR_MEMORY_LAYOUT
				i.m_pos = InnoMath::mul(Tm, Vec4(i.m_pos, 1.0f)).xyz();
#endif
			}
		...
		inline std::vector<Vertex> worldToViewSpace(const std::vector<Vertex>& rhs, Mat4 t, Mat4 r)
		{
			...
			{
				auto l_mulPos = InnoMath::worldToViewSpace(Vec4(l_vertexData.m_pos, 1.0f), t, r);
		...
		inline std::vector<Vertex> viewToWorldSpace(const std::vector<Vertex>& rhs, Mat4 t, Mat4 r)
		{
			...
			for (auto& l_vertexData : l_result)
			{
				auto l_mulPos = InnoMath::viewToWorldSpace(Vec4(l_vertexData.m_pos, 1.0f), t, r);
				l_vertexData.m_pos = l_mulPos.xyz();
			}
		...
		inline std::vector<Vertex> generateFrustumVerticesVS(Mat4 p)
		{
			...
			for (auto& i : rhs)
			{
				i.m_pos = InnoMath::clipToViewSpace(Vec4(i.m_pos, 1.0f), p).xyz();
			}
		...
		inline std::vector<Vertex> generateFrustumVerticesWS(Mat4 p, Mat4 r, Mat4 t)
		{
			...
			for (auto& i : rhs)
			{
				i.m_pos = InnoMath::viewToWorldSpace(Vec4(i.m_pos, 1.0f), t, r).xyz();
			}
