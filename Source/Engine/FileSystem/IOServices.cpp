...
bool InnoFileSystemNS::setupWorkingDirectory()
{
	...
	g_pModuleManager->getLogSystem()->printLog(LogType::INNO_DEV_VERBOSE, "FileSystem: current working directory is " + m_workingDir);
...
std::string InnoFileSystemNS::loadTextFile(const std::string & fileName)
{
	...
	if (!file.is_open())
	{
		g_pModuleManager->getLogSystem()->printLog(LogType::INNO_ERROR, "FileSystem: can't open text file : " + fileName + "!");
		return std::string();
	}
...
std::vector<char> InnoFileSystemNS::loadBinaryFile(const std::string & fileName)
{
	...
	if (!file.is_open())
	{
		g_pModuleManager->getLogSystem()->printLog(LogType::INNO_ERROR, "FileSystem: can't open binary file : " + fileName + "!");
