...
bool InnoInputSystemNS::initialize()
{
	if (InnoInputSystemNS::m_objectStatus == ObjectStatus::Created)
	{
		...
		g_pModuleManager->getLogSystem()->printLog(LogType::INNO_DEV_SUCCESS, "InputSystem has been initialized.");

		return true;
	}
	else
	{
		g_pModuleManager->getLogSystem()->printLog(LogType::INNO_ERROR, "InputSystem: Object is not created!");
		return false;
	}
}
...
bool InnoInputSystemNS::terminate()
{
	...
	g_pModuleManager->getLogSystem()->printLog(LogType::INNO_DEV_SUCCESS, "InputSystem has been terminated.");
