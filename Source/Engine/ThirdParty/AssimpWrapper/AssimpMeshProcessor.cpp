...
MeshComponent* AssimpMeshProcessor::CreateMeshComponent(const aiScene* scene, const char* exportName, uint32_t meshIndex)
{
	auto l_mesh = scene->mMeshes[meshIndex];
	
	Log(Verbose, "Creating MeshComponent for: ", l_mesh->mName.C_Str());
	
	// Convert mesh data to our format
	...
	auto l_tempEntity = g_Engine->Get<EntityManager>()->Spawn(false, ObjectLifespan::Frame, "TempMeshEntity");
	
	// Create MeshComponent via ComponentManager
	auto l_meshComponent = g_Engine->Get<ComponentManager>()->Spawn<MeshComponent>(l_tempEntity, true, ObjectLifespan::Scene);
	l_meshComponent->m_UUID = Randomizer::GenerateUUID();
	l_meshComponent->m_ObjectStatus = ObjectStatus::Created;
	
	// Save MeshComponent without subfolders and using GetTypeName()
	auto l_workingDir = g_Engine->Get<IOService>()->getWorkingDirectory();
	auto l_meshName = std::string(exportName) + "_mesh_" + std::to_string(meshIndex);
	auto l_meshPath = l_workingDir + "Data/Components/" + l_meshName + "." + MeshComponent::GetTypeName() + ".inno";
	bool result = AssetService::Save(l_meshPath.c_str(), *l_meshComponent, l_vertices, l_indices);
	...
	if (result)
	{
		Log(Success, "Created and saved MeshComponent: ", l_meshPath.c_str());
		return l_meshComponent;
	}
	else
	{
		Log(Error, "Failed to save MeshComponent: ", l_meshPath.c_str());
		return nullptr;
	}
}
