...
The Innocence Engine itself is distributed under the MIT License (see `LICENSE`
at the repository root). When the engine is built with optional integrations
or links bundled third-party SDKs, the additional license terms below apply
to the corresponding components.

This file aggregates only those notices that require redistributed source-tree
text. Permissively-licensed dependencies (MIT, BSD, zlib) propagate their
notices via their own headers under `Source/External/GitSubmodules/<name>/`.

## NVIDIA RayTracingDenoiser (NRD) SDK

> Bundled at `Source/External/GitSubmodules/NRD/`. Only linked when CMake
> option `BUILD_WITH_NRD=ON` (default). When `OFF` the submodule is not
> compiled into the binary.

This software contains source code provided by NVIDIA Corporation.

The full NVIDIA RTX SDKs license text follows. The authoritative copy lives
at `Source/External/GitSubmodules/NRD/LICENSE.txt`; this section reproduces
it for compliance with §1.c of the same license.
